...
void itable_data_write ( ofstream &output, int m, int n, int table[] );
void itable_header_write ( char *output_file_name, ofstream &output, 
  int m, int n );
void itable_triangulation_write ( char *input_file_name, char *output_file_name, 
  int m1, int n1, int m2, int n2, int table[] );
...
//  Usage:
//
//    table_delaunay input_file_name
//
{ 
  int i;
  char *eps_file_name;
  char input_file_name[255]="points.txt";
  int m1;
  int m2;
  int node_num;
  bool node_show;
...
//
  ...
  output_file_name = file_name_ext_swap ( input_file_name, "delaunay.txt" );
//
//  Read the point coordinates.
//
  dtable_header_read ( input_file_name, &m1, &node_num );

  cout << "\n";
  cout << "  Read the header of \"" << input_file_name << "\"\n";
  cout << "\n";
  cout << "  The spatial dimension of the data = " << m1 << "\n";
  cout << "  The number of points N = " << node_num << "\n";

  if ( m1 != 2 )
  {
    cout << "\n";
    cout << "HANDLE - Fatal error!\n";
    cout << "  The input spatial dimension is not 2.\n";
    exit ( 1 );
  }

  table = dtable_data_read ( input_file_name, m1, node_num );

  cout << "\n";
  cout << "  Read the data of \"" << input_file_name << "\"\n";

  r8mat_transpose_print_some ( m1, node_num, table, 1, 1, 5, 5, "  5 by 5 portion of node data read from file:" );
//
...
//
  ...
  triangle_neighbor = new int[m2*3*node_num];
...
//
  ...
  i4mat_transpose_print_some ( m2, triangle_num, triangle_node, 1, 1, 3, 5, "  3 by 5 portion of triangulation data:" );
//
//  Write the triangulation to a file.
//
  itable_triangulation_write ( input_file_name, output_file_name, 
...
//
  eps_file_name = file_name_ext_swap ( input_file_name, "delaunay.eps" );

  node_show = true;
  triangle_show = true;

  triangulation_order3_plot_eps ( eps_file_name, node_num, table, 
    triangle_num, triangle_node, node_show, triangle_show );

  cout << "\n";
  cout << "  Draw a picture of the triangulation data in \""
       << eps_file_name << "\".\n";
//
//  Terminate execution.
//
...
void itable_header_write ( char *output_file_name, ofstream &output, int m, 
  int n )
...
//
{
  ...
  output << "#  created by itable_write in table_io.C" << "\n";
  output << "#  at " << s << "\n";
  output << "#\n";
...
//  Parameters:
//
//    Input, char *INPUT_FILE_NAME, the input file name.
//
//    Input, char *OUTPUT_FILE_NAME, the output file name.
//
//    Input, int M1, the spatial dimension of the nodes (should be 2).
//
//    Input, int N1, the number of nodes.
//
//    Input, int M2, the "spatial" dimension of the triangles (should be 3).
//
//    Input, int N, the number of triangles.
//
//    Input, int TABLE[M2*N2], the triangulation data.
//
{
  ofstream output;
  ...
  {
    cout << "\n";
    cout << "ITABLE_TRIANGULATION_WRITE - Fatal error!\n";
    cout << "  Could not open the output file.\n";
    return;
  }

  itable_header_write ( output_file_name, output, m2, n2 );

  /*output << "#  Node data from \"" << input_file_name << "\"\n";
  output << "#  The node spatial dimension was " << m1 << "\n";
  output << "#  The number of nodes was " << n1 << "\n";
  output << "#\n";
  output << "#  The triangulation was computed by DTRIS2.\n";
  output << "#  The data was written to \"" << output_file_name << "\".\n";
  output << "#  by ITABLE_TRIANGULATION_WRITE.C.\n";
  output << "#\n";*/

  itable_data_write ( output, m2, n2, table );

  output.close ( );

  FILE *fp=fopen("paraN.txt", "w");
  fprintf(fp, "%d\n", n2);
  fclose(fp);
